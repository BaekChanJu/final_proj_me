...
public interface CalendarRepository extends CrudRepository<CalendarVO, Integer> {
	...
	@Query(value="UPDATE calendar SET cal_reserve = 1, checkout_date=sysdate(), price=ABS(TIMESTAMPDIFF(HOUR, cal_end, cal_start))*10000, checkout_name=?2, room_id=?3 WHERE cal_id = ?1", nativeQuery = true)
	Integer reservation(Integer calId, String tempidString, String uuid);
